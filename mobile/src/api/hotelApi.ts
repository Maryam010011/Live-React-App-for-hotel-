...
export const fetchHotels = async (city?: string): Promise<Hotel[]> => {
  const queryParam = city && city.trim() !== '' ? `?city=${encodeURIComponent(city.trim())}` : '';
  const endpoint = `${API_BASE_URL}${ENDPOINTS.HOTELS}${queryParam}`;
  ...
  try {
    ...
    console.warn('⚠️ [hotelApi] API failed, falling back to mock hotels:', error);
    if (city && city.trim() !== '') {
...
export const fetchHotelById = async (id: number | string): Promise<Hotel | null> => {
  const endpoint = `${API_BASE_URL}${ENDPOINTS.HOTELS}/${id}`;
  try {
    const response = await fetch(endpoint);
    if (!response.ok) {
    ...
    console.warn(`⚠️ [hotelApi] fetchHotelById(${id}) failed, falling back to mock:`, error);
...
export const createHotel = async (hotelData: Partial<Hotel>): Promise<Hotel> => {
  ...
  const response = await fetch(`${API_BASE_URL}${ENDPOINTS.HOTELS}`, {
    method: 'POST',
    headers: {
  ...
  const json = await response.json();
  if (!response.ok) {
    throw new Error(json.message || 'Failed to create hotel');
  }
  return json.data || json;
};
...
export const updateHotel = async (
  ...
  const authHeaders = await getAuthHeaders();
  const response = await fetch(`${API_BASE_URL}${ENDPOINTS.HOTELS}/${id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
      ...authHeaders,
  ...
  const json = await response.json();
  if (!response.ok) {
    throw new Error(json.message || 'Failed to update hotel');
  }
  return json.data || json;
};
